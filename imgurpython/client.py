...
from .helpers.format import (
    build_gallery_images_and_albums,
    build_notification,
    build_notifications,
    format_comment_tree,
)
    ...
    def refresh(self):
        ...
        if response.status_code != 200:
            raise ImgurClientError(
                "Error refreshing access token!", response.status_code
            )
...
class ImgurClient(object):
    ...
    def get_auth_url(self, response_type="pin"):
        return "%soauth2/authorize?client_id=%s&response_type=%s" % (
            self.api_url,
            self.client_id,
            response_type,
        )
    ...
    def prepare_headers(self, force_anon=False):
        ...
        if force_anon or self.auth is None:
            ...
            headers["Authorization"] = (
                "Bearer %s" % self.auth.get_current_access_token()
            )
    ...
    def make_request(self, method, route, data=None, force_anon=False):
        ...
        url = (MASHAPE_URL if self.mashape_key is not None else self.api_url) + (
        ...
        if (
            ...
            and "error" in response_data["data"]
        ):
    ...
    def validate_user_context(self, username):
        if username == "me" and self.auth is None:
            raise ImgurClientError(
                "'me' can only be used in the authenticated context."
            )
    ...
    def get_gallery_favorites(self, username, page=0):
        self.validate_user_context(username)
        gallery_favorites = self.make_request(
            "GET", "account/%s/gallery_favorites/%d" % (username, page)
        )
    ...
    def get_account_favorites(self, username, page=0):
        self.validate_user_context(username)
        favorites = self.make_request(
            "GET", "account/%s/favorites/%d" % (username, page)
        )
    ...
    def get_account_submissions(self, username, page=0):
        self.validate_user_context(username)
        submissions = self.make_request(
            "GET", "account/%s/submissions/%d" % (username, page)
        )
    ...
    def change_account_settings(self, username, fields):
        post_data = {
            ...
            for setting in set(self.allowed_account_fields).intersection(fields.keys())
        }
    ...
    def get_account_comments(self, username, sort="newest", page=0):
        self.validate_user_context(username)
        comments = self.make_request(
            "GET", "account/%s/comments/%s/%s" % (username, sort, page)
        )
    ...
    def get_account_comment_ids(self, username, sort="newest", page=0):
        self.validate_user_context(username)
        return self.make_request(
            "GET", "account/%s/comments/ids/%s/%s" % (username, sort, page)
        )
    ...
    def create_album(self, fields):
        post_data = {
            field: fields[field]
            for field in set(self.allowed_album_fields).intersection(fields.keys())
        }
    ...
    def album_remove_images(self, album_id, ids):
        ...
        return self.make_request(
            "DELETE", "album/%s/remove_images" % album_id, {"ids": ids}
        )
    ...
    def get_custom_gallery(self, gallery_id, sort="viral", window="week", page=0):
        gallery = self.make_request(
            "GET", "g/%s/%s/%s/%s" % (gallery_id, sort, window, page)
        )
    ...
    def gallery(
        self, section="hot", sort="viral", page=0, window="day", show_viral=True
    ):
        if section == "top":
            response = self.make_request(
                "GET",
                "gallery/%s/%s/%s/%d?showViral=%s"
                % (section, sort, window, page, str(show_viral).lower()),
            )
        else:
            response = self.make_request(
                "GET",
                "gallery/%s/%s/%d?showViral=%s"
    ...
    def memes_subgallery(self, sort="viral", page=0, window="week"):
        if sort == "top":
            response = self.make_request(
                "GET", "g/memes/%s/%s/%d" % (sort, window, page)
            )
        else:
    ...
    def subreddit_gallery(self, subreddit, sort="time", window="week", page=0):
        if sort == "top":
            response = self.make_request(
                "GET", "gallery/r/%s/%s/%s/%d" % (subreddit, sort, window, page)
            )
        else:
            response = self.make_request(
                "GET", "gallery/r/%s/%s/%d" % (subreddit, sort, page)
            )
    ...
    def gallery_tag(self, tag, sort="viral", page=0, window="week"):
        if sort == "top":
            response = self.make_request(
                "GET", "gallery/t/%s/%s/%s/%d" % (tag, sort, window, page)
            )
        else:
            response = self.make_request(
                "GET", "gallery/t/%s/%s/%d" % (tag, sort, page)
            )
    ...
    def gallery_item_tags(self, item_id):
        ...
        return [
            TagVote(item["ups"], item["downs"], item["name"], item["author"])
    ...
    def gallery_tag_vote(self, item_id, tag, vote):
        self.logged_in()
        response = self.make_request(
            "POST", "gallery/%s/vote/tag/%s/%s" % (item_id, tag, vote)
        )
        return response
    ...
    def gallery_search(self, q, advanced=None, sort="time", window="all", page=0):
        if advanced:
            data = {
                ...
                for field in set(self.allowed_advanced_search_fields).intersection(
                    advanced.keys()
                )
            }
        else:
        ...
        response = self.make_request(
            "GET", "gallery/search/%s/%s/%s" % (sort, window, page), data
        )
        return build_gallery_images_and_albums(response)
    ...
    def report_gallery_item(self, item_id, reason):
        self.logged_in()
        return self.make_request(
            "POST", "gallery/%s/report" % item_id, {"reason": reason}
        )
    ...
    def gallery_comment(self, item_id, comment):
        self.logged_in()
        return self.make_request(
            "POST", "gallery/%s/comment" % item_id, {"comment": comment}
        )
    ...
    def upload(self, fd, config=None, anon=True):
        ...
        data.update(
            {
                meta: config[meta]
                for meta in set(self.allowed_image_fields).intersection(config.keys())
            }
        )
    ...
    def update_image(self, image_id, fields):
        ...
        post_data = {
            field: fields[field]
            for field in set({"title", "description"}).intersection(fields.keys())
        }
    ...
    def get_conversation(self, conversation_id, page=1, offset=0):
        ...
        conversation = self.make_request(
            "GET", "conversations/%d/%d/%d" % (conversation_id, page, offset)
        )
        return Conversation(
            conversation["id"],
            conversation["last_message_preview"],
    ...
    def gallery_topic(self, topic_id, sort="viral", page=0, windown="week"):
        if sort == "top":
            response = self.make_request(
                "GET", "topics/%s/%s/%s/%d" % (topic_id, sort, window, page)
            )
        else:
            response = self.make_request(
                "GET", "topics/%s/%s/%d" % (topic_id, sort, page)
            )
